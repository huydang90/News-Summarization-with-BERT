...
class BertData():
    ...
    def preprocess(self, src, tgt, sent_labels, use_bert_basic_tokenizer=False, is_test=False):
        ...
        # return None for test and empty articles
        if ((not is_test) and len(src) == 0):
            return None

        original_src_txt = [' '.join(s) for s in src]
        
        # count number of sentences if length of sentence greater than 5 words
        ...
        # sentences of article text must not contain  >200 tokens  
        src = [src[i][:self.args.max_src_ntokens_per_sent] for i in idxs]
        sent_labels = [_sent_labels[i] for i in idxs]
        # article text must not contain >100 sentences
        src = src[:self.args.max_src_nsents]
        # limit sentence labels to max number of sentences allowed
        sent_labels = sent_labels[:self.args.max_src_nsents]
        
        # return None for test articles and articles with less than 3 sentences 
        ...
        # prepend and append the sentences with [CLS] and [SEP] respectively
        src_subtokens = [self.cls_token] + src_subtokens + [self.sep_token]
        # convert tokens to integers based on their position in vocab
        src_subtoken_idxs = self.tokenizer.convert_tokens_to_ids(src_subtokens)
        # get the index for the end of each sentence
        _segs = [-1] + [i for i, t in enumerate(src_subtoken_idxs) if t == self.sep_vid]
        # calculate the distance (number of tokens) between sentences 
        segs = [_segs[i] - _segs[i - 1] for i in range(1, len(_segs))]
        segments_ids = []
        # create alternating labels of 0 and 1s for the sentences; number of Os and 1s depends on the number of tokens contained in each sentence. This helps to distinguish between previous and subsequent sentence  
        for i, s in enumerate(segs):
            if (i % 2 == 0):
                segments_ids += s * [0]
            else:
                segments_ids += s * [1]
        # get the index for the beginning of each sentence
        cls_ids = [i for i, t in enumerate(src_subtoken_idxs) if t == self.cls_vid]
        #limit sentence labels to be the same length as number of sentences
        sent_labels = sent_labels[:len(cls_ids)]

        tgt_subtokens_str = '[unused0] ' + ' [unused2] '.join(
            [' '.join(self.tokenizer.tokenize(' '.join(tt), use_bert_basic_tokenizer=use_bert_basic_tokenizer)) for tt in tgt]) + ' [unused1]'
        # limit summary text to have not more than 500 words
        tgt_subtoken = tgt_subtokens_str.split()[:self.args.max_tgt_ntokens]
        # and not less than 5 words
        if ((not is_test) and len(tgt_subtoken) < self.args.min_tgt_ntokens):
            return None
        
        # convert summary tokens to integers
        tgt_subtoken_idxs = self.tokenizer.convert_tokens_to_ids(tgt_subtoken)
        
        # get summary and article texts with their imposed restrictions
        tgt_txt = '<q>'.join([' '.join(tt) for tt in tgt])
        src_txt = [original_src_txt[i] for i in idxs]
        
        # return ids of article words; sentence labels; ids of summary words, segment embeddings; ids that earmark the position of beginning of sentences; article text; summary text 
...
def _format_to_bert(params):
    ...
    for d in jobs:
        ...
        # expand b_data to return generated features
